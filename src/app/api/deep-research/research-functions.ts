...
export async function search(
  ...
  try {
    // Wait before search to avoid rate limits
    await delay(300);
...
export async function processSearchResults(
  ...
  // Process extractions sequentially with delays instead of parallel
  const newFindings: ResearchFindings[] = [];
  
  for (const result of searchResults) {
    try {
      await delay(200); // Wait 200ms between extractions
      const extracted = await extractContent(result.content, result.url, researchState, activityTracker);
      if (extracted) {
        newFindings.push({
